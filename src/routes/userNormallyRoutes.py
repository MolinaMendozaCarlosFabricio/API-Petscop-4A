...
user_normally_blueprint = Blueprint('usersNormally', __name__, url_prefix='/user_normally')

@user_normally_blueprint.route('/register', methods=['POST'])
def crear_usuario_ruta():
    data = request.get_json()
    return crear_usuario_normally(data)

@user_normally_blueprint.route('/get_by_id/<int:id_user>', methods=['GET'])
def get_normally_user_by_id_route (id_user):
    return get_normally_user_by_id(id_user)

@user_normally_blueprint.route('/search', methods=['POST'])
...
@user_normally_blueprint.route('/delete/<int:id_user>', methods=['DELETE'])
